<template>
  ...
  <div class="order-container">
    <h2 class="section-title">📦 Gestionar Pedido</h2>

    <div class="panel">
      <h3 class="panel-title">🧾 Detalles del Pedido</h3>

      <div class="panel-row">
        <span class="label">#️⃣ Número de pedido:</span>
        <span class="value">{{ order.orderNumber }}</span>
      </div>

      <div class="panel-row">
        <span class="label">👤 Comprador:</span>
        <Button @click="$router.push(`/perfil/${order.buyerId}`)">{{ order.buyerName }}</Button>
      </div>

      <div class="panel-row">
        <span class="label">🏠 Dirección:</span>
        <span class="value">{{ order.shippingAddress?.direccion }}, {{ order.shippingAddress?.ciudad }}, {{ order.shippingAddress?.provincia }}</span>
      </div>
      ...
      <div class="panel-row status-row">
        ...
        <span>{{ order.status === 'PAID' ? 'Pendiente' : order.status === 'SHIPPED' ? 'Enviado' : order.status === 'ARRIVED' ? 'Completado': order.status}}</span>

        <button v-if="order.status === 'PAID'" class="confirm-button" @click="marcarComoEnviado(order.orderNumber)">
          Marcar como enviado
        </button>
      </div>
    ...
    <div class="panel">
      ...
      <div class="panel-row">
        <span class="label"> Precio Envio:</span>
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      if (response?.result === 'SUCCESS') {
        ...
        const bookList = offers.map(offer => {
          ...
          return {
            title: edition?.data?.book_title || offer.data?.edition_book_name || 'Título desconocido',
    ...
    const marcarComoEnviado = async (pedidoId) => {
      const isConfirmed = window.confirm("¿Estás seguro de que quieres marcar este pedido como enviado?");
      
      if (isConfirmed) {
        await orderStore.confirmShipment(pedidoId);
        console.log("Pedido marcado como enviado.");
        window.confirm("¡Envío confirmado correctamente!");
        window.location.reload();

      } else {
        console.log("Operación cancelada. El estado del pedido no ha cambiado.");
      }
    };
